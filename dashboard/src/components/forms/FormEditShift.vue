    ...
    <div class="border-t border-border-light p-5 space-y-5">
      <div v-for="(item, index) in formState?.hours" :key="index">
        <div class="flex items-center gap-4 justify-between">
          ...
          <!-- Remove Button -->
          <button type="button" v-if="formState?.hours?.length > 1" @click="formState?.hours?.splice(index, 1)">
      ...
      <div>
        <button type="button" @click="formState?.hours.push({ start: '09:00', end: '17:00' })"
...
import type { TeamMemberShift } from '@/stores/shifts'
import { ref, watch } from 'vue'

const props = defineProps<{
  shift: TeamMemberShift
}>()

const emit = defineEmits<{
  (e: 'close'): void
  (e: 'save', value: TeamMemberShift): void
}>()

// Local state copy
const formState = ref<TeamMemberShift>({ ...props.shift })

// Keep formState in sync when props.shift changes (but not overwrite while editing)
